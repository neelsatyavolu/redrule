...
fn is_still_running(app: MeetingApp, snapshot: &DetectionSnapshot) -> bool {
    match app {
        ...
        MeetingApp::Teams => native_holds_mic(app, snapshot) || browser_holds_mic(snapshot),
        MeetingApp::Slack | MeetingApp::Webex | MeetingApp::FaceTime => native_holds_mic(app, snapshot),
