...
export function renderNote({note:n,transcript}) {
  ...
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow,noarchive"><title>${escape(n.title)} · Minutes</title><link rel="stylesheet" href="/style.css"></head><body><main><header><a href="/">Minutes</a><span>Shared meeting notes</span></header><article><p class="eyebrow">ON THE RECORD</p><h1>${escape(n.title)}</h1><p class="summary">${escape(n.tldr)}</p>${n.sections.map(s=>`<section><h2>${escape(s.heading)}</h2>${list(s.bullets)}</section>`).join('')}${n.decisions.length?`<section><h2>Decisions</h2>${list(n.decisions)}</section>`:''}${n.actionItems.length?`<section><h2>Action items</h2><ul class="actions">${n.actionItems.map(a=>`<li>${a.owner?`<span class="owner">${escape(a.owner)}</span>`:''}${escape(a.task)}</li>`).join('')}</ul></section>`:''}${transcript?`<details><summary>Transcript <span>${transcript.length} passages</span></summary>${transcript.map(s=>`<div class="passage"><div class="speaker">${escape(s.speaker)}<time>${escape(s.time)}</time></div><p>${escape(s.text)}</p></div>`).join('')}</details>`:''}</article><footer>Shared with Minutes · Anyone with this link can read this page.</footer></main></body></html>`;
}
